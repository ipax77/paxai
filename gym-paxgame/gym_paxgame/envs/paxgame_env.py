...
class PaxGameEnv(gym.Env):
    ...
    def get_osbervation_spec(self):
        ...
        state_spec = array_spec.ArraySpec(shape=(num_actions * 2, ), dtype=bool, name='mask')
        mask_spec = array_spec.ArraySpec(shape=(BUNITS*BSIZEX*BSIZEY + BUPGRADES, ), dtype=bool, name='mask')
    ...
    def step(self, action):
        ...
        if isvalid:
            ...
            # print("invalid action :( - " + str(len(self.state['actions'][p])) + " " + str(move))
            fsmove = move
            while not isvalid:
                fsmove += 1
                if fsmove >= num_actions:
                    isvalid = self.state_generator(move - (fsmove - num_actions))
                else:
                    isvalid = self.state_generator(fsmove)
            move = fsmove
            self.state['actions'][p].append(move)
            # return np.array(self.state['board']), -1, True, {}
        ...
        if (isSinglePlayer and self.state['minerals'][p] >= roundmins):
            if isSinglePlayer:
    ...
    def reset(self):
        self.state = {}
        self.state['board'] = np.zeros((20, 18 + 2), dtype=np.uint8)
        self.state['on_move'] = 1
        self.state['moves'] = {}
        self.state['moves'][1] = np.zeros(BUNITS*BSIZEX*BSIZEY + BUPGRADES, dtype=bool)
        self.state['moves'][-1] = np.zeros(BUNITS*BSIZEX*BSIZEY + BUPGRADES, dtype=bool)
        self.state['hp'] = {}
        self.state['hp'][1] = 1
        self.state['hp'][-1] = 1
    ...
    def state_generator(self, move):
        ...
        if move >= BPOS:
            ...
            if ability == 0:
                ...
                minerals = 100 # armor
            if self.state['on_move'] == 1:
                if self.state['board'][18][ability] == 0:
                    self.state['board'][18][ability] = 4
                else:
                    valid = False
            else:
                if self.state['board'][18][(ability - BSIZEY) * -1] == 0:
                    self.state['board'][18][(ability - BSIZEY) * -1] = 14
                else:
                    valid = False
        else:
    ...
    def GenRandomBuild(self, build, mono, minerals):
        ...
        if not build:
            ...
            # build = str(random.choice(['random', 'line']))
            build = 'random'
