...
def step(move, board, player, minerals):
    ...
    while valid == False:
        ...
        i += 1
        if player == +1 and (mymove[0] >= 20 or mymove[1] >= 60):
            continue

        if player == -1 and (mymove[0] < 20 or mymove[1] < 60):
            continue
        
        if board[mymove[0]][mymove[1]] == player:
            continue
...
def play(board, player_objs, count):
    ...
    while game_end is None:
        ...
        if player_objs[player].minerals <= 0 and player_objs[player * -1].minerals <= 0:
            ...
            response = requests.post(url = "http://localhost:5000/get2dresult/" + str(MAXMINS), data = json.dumps(board.tolist()), headers = headers)  
            #response = requests.post(url = "http://localhost:50586/get2dresult/" + str(MAXMINS), data = json.dumps(board.tolist()), headers = headers)  
...
class Agent(Player):
    ...
    def predict_q(self, board):
        return self.model.predict(
            np.array([board.ravel()])).reshape(20, 60)
    ...
    def move(self, board):
        ...
        move = np.unravel_index(np.argmax(temp_q), (20, 60))
        value = temp_q.max()
    ...
    def reward(self, reward_value):
        ...
        new_q[self.last_move[0]][self.last_move[1]] = reward_value
...
agent.model = tf.keras.models.load_model('model_h5/paxgame_q_2d_1000.h5')
agent.model.summary()

results = [play(new_board(3), {+1: agent, -1: RandomPlayer()}, i) for i in range(2000)]
collections.Counter(results)

agent.model.save('model_h5/paxgame_q_2d_1000.h5')
t2 = datetime.datetime.now()
print (t2 - t1)

show(results, size=200)